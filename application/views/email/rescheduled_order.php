<html>
<body>

<p>Dear ,</p>

<p>
	Rescheduling had occured for transaction ID : <?php print $delivery_id;?>
</p>
<p>
	Below is the summary of transaction:
</p>
<p>
	Transaction ID : <?php print $merchant_trans_id;?><br />
	Delivery ID : <?php print $delivery_id;?><br />
	Buyer name : <?php print $buyerfullname?><br />
	Email : <?php print $email?><br />
	Phone : <?php print $phone?><br />
	Mobile : <?php print $mobile?><br />
	<br />
	New Delivery Date : <?php print $new_date?><br />
	New Shipping Address : <?php print $shipping_addr?><br />
	New Recipient : <?php print $recipient?><br />
</p>
	<?php
	if($detail):?>
<p>
	Order Detail :<br />
	<?php print $detail->generate();?>
</p>
	<?php endif ?>
<p>
	Delivery date changes and rescheduling shall proceed under both merchant and buyer consent, therefore, 
	if you think this changes is without your consent and approval, please contact you merchant to resolve and get a date and time you can agreed upon. 
</p>
<?php if($app->signature != "" ): ?>
<p>
	Thank you,
	<?php print $app->signature;?><br /><br />
	Supported by Jayon Express team
</p>
<?php else:?>
<p>
	Thank you,
	Jayon Express team
</p>
<?php endif;?>

</body>
</html>