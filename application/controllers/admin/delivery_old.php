...
class Delivery extends Application
{
	...
	public function ajaxincoming(){
		...
		foreach($result as $value => $key)
		{
			...
			$aadata[] = array(
				...
				($key['status'] == 'canceled')?$reschedule.' '.$revoke:$reschedule.' '.$revoke.' '.$cancel
				//$key['reschedule_ref'],
				//$key['revoke_ref'],
				//($key['status'] === 'confirm')?$assign:''.' '.$edit.' '.$delete
			);
	...
	public function ajaxinprogress(){
		...
		$count_display_all = $this->db->where('status !=','pending')->where('status !=','cancel')->where('status !=','purged')->where($this->config->item('incoming_delivery_table').'.merchant_id',$this->session->userdata('userid'))->count_all_results($this->config->item('incoming_delivery_table'));
		...
		$data = $this->db->where('status !=','pending')->where('status !=','cancel')->where('status !=','purged')->where($this->config->item('incoming_delivery_table').'.merchant_id',$this->session->userdata('userid'))->limit($limit_count, $limit_offset)->order_by($columns[$sort_col],$sort_dir)->get($this->config->item('incoming_delivery_table'));
		...
		foreach($result as $value => $key)
		{
			...
			$aadata[] = array(
				...
				//$key['reschedule_ref'],
				//$key['revoke_ref'],
				($key['status'] === 'canceled')?$reschedule.' '.$purge:$printslip.' '.$cancel
			);
		}
	...
	public function ajaxcancel(){
		...
		public function ajaxreschedule(){
		...
		if(is_array($delivery_id)){
			foreach ($delivery_id as $d) {
				$this->db->where('delivery_id',$d)->update($this->config->item('incoming_delivery_table'),array('status'=>'rescheduled','change_actor'=>$actor));
				$this->do_revoke_reschedule($d,'rescheduled','incoming');
			}
		}else{
			$this->db->where('delivery_id',$delivery_id)->update($this->config->item('incoming_delivery_table'),array('status'=>'rescheduled','change_actor'=>$actor));
			$this->do_revoke_reschedule($delivery_id,'rescheduled','incoming');
		}
	...
	public function ajaxrevoke(){
		...
		if(is_array($delivery_id)){
			foreach ($delivery_id as $d) {
				...
				$this->do_revoke_reschedule($d,'revoked','incoming');
			}
		}else{
			...
			$this->do_revoke_reschedule($delivery_id,'revoked','incoming');
		}
	...
	public function ajaxpurge(){
		...
		if(is_array($delivery_id)){
			foreach ($delivery_id as $d) {
				$this->db->where('delivery_id',$d)->update($this->config->item('incoming_delivery_table'),array('status'=>'purged','change_actor'=>$actor));
			}
		}else{
			$this->db->where('delivery_id',$delivery_id)->update($this->config->item('incoming_delivery_table'),array('status'=>'purged','change_actor'=>$actor));
		}
...
/*rescheduled*/
	...
	private function do_revoke_reschedule($delivery_id,$status,$table){
