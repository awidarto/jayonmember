...
* --> filename not recognised, should be lowercased in ubuntu ( Andy Awidarto)
*/
...
class AG_Auth
{
	...
	*/
	public function register($username, $password, $email, $group_id = 100)
	{
		return $this->CI->ag_auth_model->register($username, $password, $email, $group_id);
	}
	...
	*/
	...
	/** 
	* Load an buy specific view
	*
	* @access private
	* @param string
	*/
	public function buyview($page, $params = NULL)
	{
		if($params !== NULL)
		{
			$data['data'] = $params;
		...
		$data['page'] = $page;
		$this->CI->load->view($this->config['buy_views_root'].'index', $data);
	}
